    ...
    <div class="col-lg-10 mx-auto">
        <div class="card">
            ...
            <div class="card-body add-new-style">
                <form class="form-horizontal" action="{{ route('types.update', $type->id) }}" method="POST" enctype="multipart/form-data">
                	<input name="_method" type="hidden" value="PATCH">
                    @csrf
                    <div class="form-group row">
                        <label class="col-md-3 col-form-label">{{translate('Name')}}</label>
                        <div class="col-md-9">
                            <input type="text" placeholder="{{translate('Name')}}" id="name" name="name" value="{{$type->name}}" class="form-control" required>
                        </div>
                    </div>
                        ...
                        <div class="col-md-9">
                            <select class="form-control" id="firstcategory" onchange="category1(this.value);" name="first_category_id" required>
                                ...
                                @foreach ($first_categories as $category)
                                    @if($type->first_category_id == $category->id)
                        ...
                        <div class="col-md-9">
                            <select class="form-control" id="secondcategory" onchange="category2(this.value);" name="second_category_id" required>
                                ...
                                @foreach ($second_categories as $category)
                                    @if($type->second_category_id == $category->id)
                                        <option value="{{ $category->id }}" selected>{{ $category->getTranslation('name') }}</option>
                                    @else
                                        <option value="{{ $category->id }}">{{ $category->getTranslation('name') }}</option>
                                    @endif
                        ...
                        <div class="col-md-9">
                            <select class="form-control" id="thirdcategory" name="third_category_id" required>
                                ...
                                @foreach ($third_categories as $category)
                                    @if($type->third_category_id == $category->id)
                                        <option value="{{ $category->id }}" selected>{{ $category->getTranslation('name') }}</option>
                                    @else
                                        <option value="{{ $category->id }}">{{ $category->getTranslation('name') }}</option>
                                    @endif
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-md-9">
                            <div class="input-group" data-toggle="aizuploader" data-type="image">
                                ...
                                <input type="hidden" name="icon" class="selected-files" value="{{ $type->icon }}">
                            </div>
