...
<div class="aiz-titlebar text-left mt-2 mb-3 ">
	<div class="align-items-center">
			<h1 class="h3">{{translate('Service Reviews')}}</h1>
	</div>
</div>

<div class="card clkistion">
    <div class="card-header">
        <div class="row flex-grow-1">
            <div class="col">
                <h5 class="mb-0 h6">{{translate('Service Reviews')}}</h5>
                
            </div>
            <div class="col-md-6 col-xl-4 ml-auto mr-0">
                <form class="" id="sort_by_rating" action="" method="GET">
        ...
        <table class="table aiz-table mb-0">
            <thead>
                <tr>
                    ...
                    <th data-breakpoints="lg">{{translate('Service Owner')}}</th>
            ...
            <tbody>
                @foreach($reviews as $key => $review)
                    @if ($review->service != null && $review->user != null)
                        <tr>
                            <td>{{ ($key+1) + ($reviews->currentPage() - 1)*$reviews->perPage() }}</td>
                            <td>
                                <a href="{{ route('service', $review->service->slug) }}" target="_blank" class="text-reset text-truncate-2">{{ $review->service->getTranslation('name') }}</a>
                            </td>
                            <td>{{ $review->service->added_by }}</td>
