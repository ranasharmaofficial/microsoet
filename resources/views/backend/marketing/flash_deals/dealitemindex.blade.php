...
<div class="aiz-titlebar text-left mt-2 mb-3">
	<div class="row align-items-center">
		<div class="col-md-6">
			<h1 class="h3">{{translate('All Flash Deals Items')}}</h1>
		</div>
		<div class="col-md-6 text-md-right">
			<a href="{{ route('flash_deals_item.create') }}" class="btn btn-circle btn-info">
				<span>{{translate('Create New Flash Deal Item')}}</span>
			</a>
		</div>
	</div>
</div>
...
<div class="card">
    ...
    <div class="card-body">
        <table class="table aiz-table mb-0" >
            ...
            <tbody>
                @foreach($flash_deals_item as $key => $flash_deal)
                    <tr>
                        <td>{{ ($key+1) + ($flash_deals_item->currentPage() - 1)*$flash_deals_item->perPage() }}</td>
                        <td>{{ $flash_deal->title }}</td>
                        <td>{{ $flash_deal->sub_title }}</td>
                        <td><img src="{{asset('public')}}/{{ $flash_deal->image }}" alt="banner" class="h-50px"></td>
						...
						<td>{{  $flash_deal->page_link }} </td>
						<td class="text-right">
                            <a class="btn btn-soft-primary btn-icon btn-circle btn-sm" href="{{route('flash_deals_item.edit', ['id'=>$flash_deal->id, 'lang'=>env('DEFAULT_LANGUAGE')] )}}" title="{{ translate('Edit') }}">
                                <i class="las la-edit"></i>
                            </a>
                            <a href="#" class="btn btn-soft-danger btn-icon btn-circle btn-sm confirm-delete" data-href="{{route('flash_deals_item.destroy', $flash_deal->id)}}" title="{{ translate('Delete') }}">
        ...
        <div class="clearfix">
            <div class="pull-right">
                {{ $flash_deals_item->appends(request()->input())->links() }}
            </div>
        </div>
    </div>
</div>
