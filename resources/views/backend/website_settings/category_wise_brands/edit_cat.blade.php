...
<div class="card">
	<form action="{{ route('website.editcatwisebrand') }}" method="POST" enctype="multipart/form-data">
		@csrf
		<input type="hidden" name="cid" value="{{$catwisebrandid->id}}">
		...
		<div class="card-body">
			<div class="form-group row">
				...
				<div class="col-sm-10">
					...
					@foreach (\App\Models\Category::where('parent_id', '0')->get() as $key => $category)
						<option value="{{ $category->id }}" @if($catwisebrandid->category_id == $category->id) selected @endif>{{  $category->getTranslation('name') }}</option>
					@endforeach
					</select>
				</div>
			</div>
			<div class="form-group row">
				...
				<div class="col-sm-10">
					 ...
					 <option value="{{ $brand->id }}" @if($catwisebrandid->brand_id == $brand->id) selected @endif>{{ $brand->getTranslation('name') }}</option>
					 @endforeach
                                </select>
				</div>
			</div>
			<div class="form-group row">
				...
				<div class="col-sm-10">
					<div class="input-group d-block d-md-flex">
						...
						<input type="text" class="form-control w-100 w-md-auto" value="{{$catwisebrandid->url}}" placeholder="{{ translate('Enter Url') }}" name="url" required>
					</div>
			...
			<div class="form-group row">
				...
				<div class="col-sm-10">
					<div class="input-group d-block d-md-flex">
						<input type="text" class="form-control w-100 w-md-auto" value="{{$catwisebrandid->title}}" placeholder="{{ translate('Enter Title') }}" name="title" required>
					</div>
				</div>
			</div>
			 <div class="form-group row">
                            ...
                            <div class="col-md-8">
                                <div class="input-group" data-toggle="aizuploader" data-type="image">
                                    ...
                                    <input type="hidden" name="image" value="{{ $catwisebrandid->image }}" class="selected-files">
                                </div>
