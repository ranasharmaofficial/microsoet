...
@section('meta_description'){{ $news->meta_description }}@stop

@section('meta_keywords'){{ $news->meta_keywords }}@stop

@section('meta')
    <!-- Schema.org markup for Google+ -->
    <meta itemprop="name" content="{{ $news->meta_title }}">
    <meta itemprop="description" content="{{ $news->meta_description }}">
    <meta itemprop="image" content="{{ uploaded_asset($news->meta_img) }}">
    ...
    <meta name="twitter:image" content="{{ uploaded_asset($news->meta_img) }}">

    <!-- Open Graph data -->
    <meta property="og:title" content="{{ $news->meta_title }}" />
    <meta property="og:type" content="website" />
    <meta property="og:url" content="{{ route('blog.details', $news->slug) }}" />
    <meta property="og:image" content="{{ uploaded_asset($news->meta_img) }}" />
               ...
               <div class="col-md-12 breadmcrumsize">
                  <nav aria-label="breadcrumb">
                     <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item"><a href="{{url('news-media')}}">News</a></li>
               ...
               <div class="col-xl-9 col-wd-9gdot5  mb-5">
                  <div class="main-container fl-wrap fix-container-init blog-pages">
				  	<div class="blog-detail-banners mb-3">
										<img src="{{ uploaded_asset($news->banner) }}" alt="">
										</div>
				      <div class="section-title">
						...
						<p class="mb-3">{{$news->title}}</p>
                     </div>
					   <div class="single-post-header fl-wrap mt-3">
                                            
                                            <div class="author-link float-left"><a href="javascript:void(0);">
                                                <img src="{{static_asset('assets_web/img/auth3.jpg')}}" alt="" class="mx-2">  <span>By Admin</span></a></div>
                                            <span class="post-date float-left"><i class="fa fa-clock"></i> Updated {{date('d-M-Y',strtotime($news->created_at))}}</span>
										...
										<div class="single-post-content_text">
                                                <h4 class="mb_head my-2 mt-3">News Details</h4>
                            ...
                            <div class=" d-flex">
                                <div class="commetn-img">
                                    <img src="img/autok.jpg" alt="comment">
                                </div>
                                <div class="comment-text">
                                    <h2>Angela De</h2>
                                    <em>September 11 2020 at 900.am</em>
                                    <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse gravida sem sit amet molestie portitor. Suspendisse gravida sem sit amet molestie portitor?                                    </p>
                                    <a href="#">Reply</a>
                                </div>
                            </div>
                        </div>  --}}
...
{{-- <div class="form-comment">
                            <form id="comment">
                                ...
                                <input type="text" id="eamil" name="email" placeholder="Email Id">
                                <textarea placeholder="Massage*" id="massage"></textarea>
                                <input type="submit" class="submits" value="Submit" id="comment_id">
                            </form>
                        </div> --}}
										  </div>
									...
									<div class="tab-content1" style="">
										<div class="post-widget-container fl-wrap"> @foreach ($recent_blogs as $item)
											...
											<div class="post-widget-item fl-wrap d-flex mb-3">
												<div class="post-widget-item-media"> <a href="{{ url(" blog ").'/'. $item->slug }}"><img src="{{ uploaded_asset($item->banner) }}" alt="{{ $item->title }}"></a> </div>
