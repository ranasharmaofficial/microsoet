    ...
    public function calculateCommission($order){
        ...
        if ($order->payment_type == 'cash_on_delivery') {
            foreach ($order->orderDetails as $orderDetail) {
                ...
                $seller_details = User::where('id', $orderDetail->seller_id)->where('user_type', 'seller')->first();

                if($seller_details->seller_commission!=null){
                    $commission_percentage = $seller_details->seller_commission;
                }else{
                ...
                if ($orderDetail->product->user->user_type == 'seller') {
                    ...
                    if($orderDetail->franchise_id!=null){
                        $franchise_commission = ($admin_commission * 51) / 100;
                    }else{
                    ...
                    if($orderDetail->franchise_id!=null){
                        $TotalAdminCommission = $admin_commission-$franchise_commission;
                    }else{
                        $TotalAdminCommission = $admin_commission;
                    }
                    ...
                    $commission_history->franchise_id = $orderDetail->franchise_id;
                    $commission_history->admin_commission = $TotalAdminCommission;
                    $commission_history->franchise_commission = $franchise_commission;
