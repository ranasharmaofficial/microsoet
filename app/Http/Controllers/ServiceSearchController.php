...
class ServiceSearchController extends Controller
{
    public function index(Request $request, $category_id = null, $brand_id = null){
        ...
        $get_type_ids = $service_list->groupBy('type_id')->pluck('type_id');

        $filter_selected_types = Type::select('id', 'name', 'icon')->whereIn('id', $get_type_ids)->get();
        ...
        $data = [
            ...
            'filter_selected_types' => $filter_selected_types,
        ];
    ...
    public function getFilteredServices(Request $request){
        ...
        if($request->has('master_service_id')) {
            $product_list->where('type_id', $request->master_service_id);
        }
                ...
                <div class="product-box services">';
                    if($request->has('master_service_id')){
                        if($row->is_cart_product){
                            $output .= '<button class="shortlistedbtn addser-position-absolute">
                            ...
                            if(Auth::user() != null){
                                ...
                                </button><button class="shortlistedbtn addser-position-absolute" style="display:none;">
                                <i class="fa fa-check-circle"></i> Shortlisted
                            </button>';
                            }else{
                                $output .= '<a href="'.url('login').'" class="addservicebtn addToServiceCart addser-position-absolute">
                                    <i class="fa fa-plus-circle"></i> Shortlist
                                </a>';
                            }
                        }
                    ...
                    if(Auth::user() != null){
                        $output .= '<button class="addserviceaquote quoteModal" data-id="'.$row->id.'">Get A Quote</button>
