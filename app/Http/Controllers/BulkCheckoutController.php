...
class BulkCheckoutController extends Controller
{
    ...
    public function get_shipping_info(Request $request, $type=null)
    {
		...
		if($addressCount>=1)
		{
            if($type == 'all_category'){
                // if($type != null){
                $get_category_type = BulkCart::select('category_id', 'types')->where('user_id', Auth::user()->id)->first();
			    $carts = BulkCart::where('user_id', Auth::user()->id)->where('types', 'all_category')->get();
                // dd($carts);
                // dd('All category');
            }else{
                $get_category_type = BulkCart::select('category_id', 'types')->where('user_id', Auth::user()->id)->first();
			    $carts = BulkCart::where('user_id', Auth::user()->id)->where('types', 'from_product')->get();
                // dd($carts);
            }
            if ($carts && count($carts) > 0) {
				$categories = Category::all();
				return view('frontend.bulk_shipping_info', compact('categories', 'carts', 'get_category_type'));
			}

			flash(translate('Your cart is empty'))->success();
			return redirect()->back()->with(session()->flash('alert-danger', 'Cart Empty.'));
    ...
    public function getCheckoutAllCategory(Request $request)
    {
		if ($request->type != null) {
            return $this->get_shipping_info($request, $request->type);
        }
        abort(404);
    }
