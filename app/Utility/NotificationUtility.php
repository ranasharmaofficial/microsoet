...
class NotificationUtility
{
    ...
    {
        $user = Auth::user();
        $order_template = view('mailers.order_placed.order_placed',compact('order', 'user'));

        $user_data = array(
            "mail_identifier" => "product_order_placed_to_customer",
            "to"=> $order->user->email,
            "name"=> $order->user->name,
            "subject"=> translate('A new order has been placed User') . ' - ' . $order->code,
            "template"=> $order_template,
        );

        $vendor_data = array(
            "mail_identifier" => "product_order_placed_to_vendor",
            "to"=> $order->orderDetails->first()->product->user->email,
            "name"=> $order->orderDetails->first()->product->user->name,
            "subject"=> translate('A new order has been placed vendor') . ' - ' . $order->code,
            "template"=> $order_template,
        );
        ...
        try {
            ...
            // $download_invoice = new \App\Http\Controllers\InvoiceController;
            // $download_invoice->invoice_download($order->id);
